...
test_p53_control <- groupComparison(
  contrast.matrix = contrast_matrix["P53 KD vs control",],
  data = MSstats_processed,
  save_fitted_models = TRUE, # Save to use for power analysis
  log_base = 2,
  append = FALSE,
...
test_F_M <- groupComparison(
  contrast.matrix = contrast_matrix["F vs M",],
  data = MSstats_processed,
  save_fitted_models = TRUE, # Save to use for power analysis
  log_base = 2,
  append = FALSE,
...
sample_size_p53_control <- designSampleSize(
  data = test_p53_control$FittedModel,
  desiredFC = c(1.3, 2.5),
  FDR = 0.05,
  numSample = 8,
  power = TRUE,
  ...
  log_file_path = paste0(
    output_dir,
    "logs/designSampleSize_p53_control_log_",
    format(Sys.time(), "%Y_%m_%d_%H_%M_%S"),
    ".txt"
  )
)
sample_size_NS_S <- designSampleSize(
  ...
  log_file_path = paste0(
    output_dir,
    "logs/designSampleSize_NS_S_log_",
    format(Sys.time(), "%Y_%m_%d_%H_%M_%S"),
    ".txt"
  )
)
sample_size_F_M <- designSampleSize(
  data = test_F_M$FittedModel,
  desiredFC = c(1.3, 2.5),
  FDR = 0.05,
  numSample = 8,
  power = TRUE,
