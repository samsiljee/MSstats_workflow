...
library(splitstackshape) # Split up uique protein names for weighted input
```
...
# Process using weigthing for unique peptides using function derived from example code sent by Mateusz Staniak
...
cluster_exclusion_list <- c(8, 12, 18)
crash_exclusion_list <- c(1, 22)

indexes <- unique(MSstatsTMT_input$Cluster)[!unique(MSstatsTMT_input$Cluster) %in% 1:max(c(cluster_exclusion_list, crash_exclusion_list))]
...
MSstats_summarised <- getWeightedProteinSummary(
  dplyr::filter(MSstatsTMT_input, Cluster == 19),
  norm = "p_norm",
  norm_parameter = 1,
  weights_mode = "contributions",
  tolerance = 0.1,
