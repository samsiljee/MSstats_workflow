...
weighted_PSM_prep <- function(PSMs) {
  PSMs <- separate_longer_delim(PSMs, cols = Protein.Accessions, delim = "; ") %>%
...
PDtoMSstatsWeightedSummary <- function(PSMs, annotations) {
  ...
  # Add protein clusters and add back Intensity column
  MSstatsTMT_input <- addClusterMembership(MSstatsTMT_input, pp_graph)

  # Select the columns as needed in the correct order
  MSstatsTMT_input <- select(
    MSstatsTMT_input,
    ProteinName,
    PeptideSequence,
    Charge,
...
# New method to format the input
MSstatsTMT_input <- MSstatsTMT::PDtoMSstatsTMTFormat(
  input = weighted_PSM_prep(PSMs),
