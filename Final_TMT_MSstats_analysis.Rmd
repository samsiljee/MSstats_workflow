...
## Full peptide-protein graph
# Get list of all protein.peptide combos
peptide_proteins <- unique(as.data.table(PSMs)[, .(PeptideSequence = Annotated.Sequence, ProteinName = Protein.Accessions)])
# Separate out protein groups into individual rows
peptide_proteins <- separate_longer_delim(peptide_proteins, cols = ProteinName, delim = "; ") %>%
  mutate(ProteinName = str_replace_all(ProteinName, "\\-1", "")) %>%
  as.data.table()
# Make the graph
peptide_proteins_graph <- createPeptideProteinGraph(peptide_proteins)

# Cluster identification
peptide_proteins <- addClusterMembership(peptide_proteins, peptide_proteins_graph)
# Recombine with PSMs to make full input
all_proteins_input <- merge(
  as.data.table(PSMs),
  peptide_proteins,
  by.x = "Annotated.Sequence",
  by.y = "PeptideSequence",
  allow.cartesian = T,
  all.x = T,
  all.y = T
)
# Remove the protein accessions columns
all_proteins_input <- subset(all_proteins_input, select = -c(Protein.Accessions, Master.Protein.Accessions)) %>% as.data.frame()
...
MSstatsTMT_input = as.data.table(MSstatsTMT_input)

# Process isoforms
MSstatsTMT_input[, log2Intensity := log(Intensity, 2)]
processed_input_graph = createPeptideProteinGraph(MSstatsTMT_input)
MSstatsTMT_input = addClusterMembership(MSstatsTMT_input, processed_input_graph)
MSstatsTMT_input_iso = processIsoforms(MSstatsTMT_input, T, T, F)

# Normalization ----
MSstatsTMT_input_iso[, Intensity := 2 ^ log2Intensity]
MSstatsTMT_input_iso = normalizeSharedPeptides(MSstatsTMT_input_iso)
MSstatsTMT_input_iso_graph = createPeptideProteinGraph(MSstatsTMT_input_iso)
MSstatsTMT_input_iso = addClusterMembership(MSstatsTMT_input_iso, MSstatsTMT_input_iso_graph)

# Cluster statistics ----
MSstatsTMT_input_iso = getClusterStatistics(MSstatsTMT_input_iso, TRUE)

# Make input for next steps
MSstatsTMT_input_tbl = MSstatsTMT_input_iso

# Protein cluster processing and descriptive statistics ----
MSstatsTMT_input_each_uni = MSstatsTMT_input_tbl[(HasUnique)]
MSstatsTMT_input_each_uni[, IsUnique := uniqueN(ProteinName) == 1, by = "PSM"]
MSstatsTMT_input_each_uni[, NumProteins := uniqueN(ProteinName), by = "Cluster"]
MSstatsTMT_input_each_uni[, HasUnique := any(IsUnique), by = "ProteinName"]
MSstatsTMT_input_each_uni = MSstatsTMT_input_each_uni[(HasUnique)]
MSstatsTMT_input_each_uni[, IsUnique := uniqueN(ProteinName) == 1, by = "PSM"]
MSstatsTMT_input_each_uni[, NumProteins := uniqueN(ProteinName), by = "Cluster"]

# Save the prepared data
save(MSstatsTMT_input_each_uni, file = paste0(output_dir, "TMT_preprocessed_weighted.rda"))
```
...
MSstats_summarised <- getWeightedProteinSummary(
 ...
 # Format for group comparison input
